...
FRAMES = OUT/'frames_ascii'
FRAMES_PULSE = OUT/'frames_ascii_pulse'
...
def render_ascii(frame_rgb, idx, pulse_mode=False):
    ...
    # Futuristic-Japanese anchors: torii-like canal gate, Fuji ridge, and neon kana-like sign columns.
    torii_alpha = 82 if not pulse_mode else int(86+52*breath)
    gate_y = int(H*.66); gate_w=int(W*.23); gate_x=W//2
    d.line([(gate_x-gate_w, gate_y),(gate_x+gate_w, gate_y)], fill=(255,74,112,torii_alpha), width=4)
    d.line([(gate_x-gate_w+34, gate_y-18),(gate_x+gate_w-34, gate_y-18)], fill=(255,122,96,torii_alpha), width=2)
    d.line([(gate_x-gate_w+42, gate_y),(gate_x-gate_w+70,H-26)], fill=(255,74,112,torii_alpha), width=3)
    d.line([(gate_x+gate_w-42, gate_y),(gate_x+gate_w-70,H-26)], fill=(255,74,112,torii_alpha), width=3)
    d.line([(W//2-170,int(H*.47)),(W//2,int(H*.32)),(W//2+170,int(H*.47))], fill=(210,235,245,54), width=2)
    kana='トーキョー 未来 都市 鶴 光 雨'
    for k,ch in enumerate(kana):
        if ch==' ': continue
        x = 64 + (k*53)%260 if k<9 else W-300 + ((k-9)*47)%240
        y = int(H*.28 + ((k*71)%230))
        d.text((x,y), ch, font=large, fill=(255,82,130,62 if not pulse_mode else int(70+45*breath)))
    # Cranes: flowing overhead; use glyph geometry so they stay readable.
    crane_positions = [(-0.05,0.18,1.25),(0.13,0.115,0.92),(0.33,0.155,0.78),(0.56,0.095,0.95),(0.78,0.19,1.38),(1.02,0.145,1.05)]
    for j,(px,py,sc) in enumerate(crane_positions):
        x = int(((px + 0.18*u + 0.012*math.sin(phase+j))%1.18 - .09)*W)
        y = int((py + 0.025*math.sin(phase*1.4+j))*H)
        s = sc*(1.0 + (0.08*breath if pulse_mode else 0))
        draw_crane(d, x, y, s=s, alpha=185 if not pulse_mode else int(175+55*breath))
    ...
    d.text((30,H-34), 'FUTURE EDO // CRANE FLIGHT // ASCII SIGNAL', font=small, fill=(190,235,245,150))
...
def main():
    ...
    encode(FRAMES, OUT/'future_edo_cranes_ascii.mp4', OUT/'future_edo_cranes_ascii.gif')
    encode(FRAMES_PULSE, OUT/'future_edo_cranes_ascii_breathe.mp4', OUT/'future_edo_cranes_ascii_breathe.gif')
    # Poster frames/contact sheet.
    for name, dpath in [('ascii',FRAMES),('breathe',FRAMES_PULSE)]:
        Image.open(dpath/'f_0048.png').save(OUT/f'poster_{name}_2s.png')
    thumbs=[]
    ...
    sheet.save(OUT/'contact_sheet_ascii_video.png')
    # Manifest
    items=[]
